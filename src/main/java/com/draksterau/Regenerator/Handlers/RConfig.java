...
public final class RConfig extends RObject {
    ...
    public int activityTimeStart = 0; //00:00
    public int activityTimeEnd = 23; //06:00
    ...
    public boolean disableRegenJobsWithOnlinePlayers = false;
    ...
    // SQL settings
    public static String sqlUrl = "jdbc:mysql://localhost:3306/regenerator";
    public static String sqlTablePrefix = "regen";
    public static String sqlUser = "root";
    public static String sqlPassword = "root";
    // JDBC variables for opening and managing connection
    public static Connection connectionSQL;
    public static Statement statementSQLcon;
...
/*
        this.sqlUrl = ("jdbc:mysql://" + config.getString("sqlServer") + ":" + config.getString("sqlServerPort") + "/" + config.getString("database")); // + "?useSSL=false";
        this.sqlUser = config.getString("sqlUser");
        this.sqlPassword = config.getString("sqlPassword");*/

        // New SQL connection and creating tables
        try {
            for (World world : Bukkit.getServer().getWorlds()) {
                sqlQuery = "CREATE TABLE IF NOT EXISTS " + sqlTablePrefix + "_" + world.getName() + " (id int(11) NOT NULL auto_increment, chunkX int(10) NOT NULL, chunkZ int(10) NOT NULL, activityTime bigint(16) NOT NULL, PRIMARY KEY (id), UNIQUE(chunkX, chunkZ), KEY(activityTime));";
                // executing UPDATE query
                statementSQLcon = connectSQL();
...
//            sqlEx.printStackTrace();
            ...
            this.plugin.utils.throwMessage(MsgType.WARNING, "WARNING! Regenerator RConfig database connection error! Stoping...");
            this.plugin.disablePlugin();
            Bukkit.getServer().shutdown();
        }
...
/*        finally {
            ...
            //try { con.close(); } catch(SQLException sqlSe) { this.plugin.utils.throwMessage(MsgType.WARNING, sqlSe.getMessage()); }
            try { statementSQLcon.close(); } catch(SQLException sqlSe) { this.plugin.utils.throwMessage(MsgType.WARNING, sqlSe.getMessage()); }
            //try { rs.close(); } catch(SQLException sqlSe) { this.plugin.utils.throwMessage(MsgType.WARNING, sqlSe.getMessage()); }
        }*/
        this.activityTimeStart = config.getInt("activityTimeStart");
        this.activityTimeEnd = config.getInt("activityTimeEnd");
        ...
        if (!config.isSet("disableRegenJobsWithOnlinePlayers")) {
            this.plugin.utils.throwMessage(MsgType.NEW,String.format(this.plugin.lang.getForKey("messages.addingNewConfig"), "disableRegenJobsWithOnlinePlayers", this.configFile.getName()));
        } else {
            this.disableRegenJobsWithOnlinePlayers = config.getBoolean("disableRegenJobsWithOnlinePlayers");
        }
    }

    public static Statement connectSQL() {
        sqlUrl = ("jdbc:mysql://" + config.getString("sqlServer") + ":" + config.getString("sqlServerPort") + "/" + config.getString("database")); // + "?useSSL=false";
        sqlUser = config.getString("sqlUser");
        sqlPassword = config.getString("sqlPassword");

        try {
            // opening database connection to MySQL server
            if (connectionSQL != null && connectionSQL.isValid(43200)) {
                plugin.utils.throwMessage(MsgType.INFO, "Database connection already exists.");
            }
            else {
                if (connectionSQL != null) {connectionSQL.close();}
                connectionSQL = DriverManager.getConnection(sqlUrl, sqlUser, sqlPassword);
                plugin.utils.throwMessage(MsgType.SUCCESS, "Regenerator connection to the database was successful.");
            }

            // getting Statement
            statementSQLcon = connectionSQL.createStatement();
            plugin.utils.throwMessage(MsgType.INFO, "Regenerator database connection statement created successful.");

        } catch (SQLException sqlEx) {
//            sqlEx.printStackTrace();
            plugin.utils.throwMessage(MsgType.WARNING, sqlEx.getMessage());
            plugin.utils.throwMessage(MsgType.WARNING, "Regenerator database connection error! Stoping...");
            plugin.disablePlugin();
            Bukkit.getServer().shutdown();
        }
        return statementSQLcon;
    }
    ...
    public void saveData() {
        ...
        config.set("disableRegenJobsWithOnlinePlayers", this.disableRegenJobsWithOnlinePlayers);
        try {
