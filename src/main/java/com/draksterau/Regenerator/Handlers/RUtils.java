...
//            }
    ...
    public boolean regenerateChunk(Chunk chunk) {
        ...
        try {
            ...
            com.sk89q.worldedit.EditSession session = com.sk89q.worldedit.WorldEdit.getInstance().newEditSession(world);
...
//        plugin.availableIntergrations.add(Landlord);
    ...
    public void clearEntitiesFromChunk (RChunk rChunk) {
        ...
        for (Entity e : rChunk.getChunk().getEntities()) {
            if (plugin.config.excludeEntityTypesFromRegeneration.contains(e.getType().name())) continue;
