    ...
    public void run() {   
            ...
            if (!isManual) {
                ...
                if (!RChunk.plugin.utils.getPlayersNearChunk(RChunk, RChunk.plugin.config.distanceNearbyMinimum).isEmpty()) {
                    ...
                    // Add lastActivity (- world.regenInterval) if chank is to near to player
                    RChunk.addActivity();
                    return;
                }
                ...
                if (RChunk.getChunk().getWorld().isChunkLoaded(RChunk.chunkX, RChunk.chunkZ) && !RChunk.plugin.config.regenerateChunksInUseByPlayers) {
