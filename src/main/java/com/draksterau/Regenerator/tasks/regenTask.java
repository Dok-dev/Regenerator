...
public class regenTask extends BukkitRunnable {
    ...
    public void run() {
        ...
        // Checking conditions for allow processing of regeneration jobs.
        if (plugin.config.disableRegenJobsWithOnlinePlayers && Bukkit.getOnlinePlayers().size()>0) {
            plugin.utils.throwMessage(MsgType.DEBUG,"Regeneration this online players deny in config.");
            return;
        }
        int hours = ((int)(Math.floor(System.currentTimeMillis()/3600000))%24  + 3);
        plugin.utils.throwMessage(MsgType.DEBUG,"Now time is " + hours + "h. Activity time is "+ plugin.config.activityTimeStart+"-"+plugin.config.activityTimeEnd);
        if (hours < plugin.config.activityTimeStart || hours > plugin.config.activityTimeEnd) {
            plugin.utils.throwMessage(MsgType.DEBUG,"Now is not active time for regen job.");
            return;
        }
  ...
  //      if (rWorld.autoRegen == true) {
            List<RChunk> rChunks = rWorld.getAllRChunks((int)plugin.config.numChunksPerParse);
...
/*        } else {
            plugin.utils.throwMessage(MsgType.WARNING, String.format(plugin.lang.getForKey("messages.skippingRegenAutoRegen"),plugin.config.minTpsRegen));
            this.plugin.isParseActive = false;
            return null;
        }*/
    ...
    public List<RChunk> getNumChunksToRegen() {
        List<RChunk> chunksToRegen = new ArrayList<RChunk>();
        for (RWorld rWorld : plugin.loadedWorlds) {
            List<RChunk> rChunks = rWorld.getAllRChunks(-1);
            for (RChunk rChunk : rChunks) {
                if ((System.currentTimeMillis() - rChunk.lastActivity) >= (rWorld.regenInterval * 1000) && rChunk.lastActivity != 0 && rChunk.lastActivity != -1) {
